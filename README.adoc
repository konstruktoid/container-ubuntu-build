...
For example:
```sh
$ sudo sh buildeb.sh bionic http://archive.ubuntu.com/ubuntu "$(pwd)/buildarea"
...
docker run --privileged -v "$(pwd)":/opt/buildarea konstruktoid/ubuntubuild bionic http://archive.ubuntu.com/ubuntu
```
